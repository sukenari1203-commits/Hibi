...
// Physical map ordering: depot on the LEFT, freight station on the RIGHT.
// Shift each screen as a unit, preserving its interior left-to-right ordering.
const stationX=x=>x+88;
const depotX=x=>x-88;
const SCREEN={depot:-50,station:38};
const stationTruckX=stationX(-48),depotTruckX=depotX(36);
...
// RIGHT screen: train station and loading parking.
building(stationX(-69),-11,18,11,5.7,M.red,'貨物駅');
box(34,.35,8,mat(0x77736b,1),stationX(-58),.18,-16);
for(let x=-74;x<=-43;x+=5)box(.35,3.6,.35,M.steel,stationX(x),1.8,-13.5);
box(32,.26,.4,M.steel,stationX(-58),3.55,-13.5);
addLabel('貨物列車',stationX(-59),5,-22,'箱をドラッグしてトラックへ');
addLabel('駅側トラック駐車場',stationTruckX,3.2,8,'最大3箱・積載後自動発車');

// LEFT screen: rally on the left, parking in the middle, storage on the right.
building(depotX(61),-6,17,13,5.4,M.canvas,'補給倉庫');
...
assembly.position.set(depotX(18),0,10);scene.add(assembly);addLabel('集合地点',depotX(18),2.5,10,'後で分隊編成に使用');
const storageBase=box(18,.18,13,mat(0x726c58,1),depotX(58),.1,10);storageBase.userData.zone='storage';
for(let x=51.5;x<=64.5;x+=4.2)for(let z=6.5;z<=13.5;z+=3.5){const p=new THREE.Group();for(let i=-1;i<=1;i++)box(3.1,.18,.42,M.wood,0,.12,i*.95,p);p.position.set(depotX(x),.15,z);scene.add(p)}
addLabel('荷物置き場',depotX(58),3.2,10,'箱を置いてタップで開封');
addLabel('補給所トラック駐車場',depotTruckX,3.2,8,'ここで荷下ろし');
for(const [x,z] of [[28,23],[43,22],[70,20]]){const g=new THREE.Group();box(8,2.8,5,M.canvas,0,1.4,0,g);const r1=box(8.6,.22,3,M.roof,0,3.05,-1.1,g);r1.rotation.x=.35;const r2=box(8.6,.22,3,M.roof,0,3.05,1.1,g);r2.rotation.x=-.35;g.position.set(depotX(x),0,z);scene.add(g)}
for(let x=-4;x<=74;x+=13){box(3,.65,.55,M.dark,depotX(x),.33,27);box(.35,1.2,.35,M.dark,depotX(x-1.3),.6,27);box(.35,1.2,.35,M.dark,depotX(x+1.3),.6,27)}

const train=new THREE.Group();
function wheel(x,z,r=.58,parent=train){const w=cyl(r,r,.38,12,M.dark,x,.58,z,parent);w.rotation.x=Math.PI/2;return w}
function makeLocomotive(){const g=new THREE.Group();box(8,.9,3.6,M.dark,0,.72,0,g);const boiler=cyl(1.25,1.25,5.8,14,M.steel,0,2.05,0,g);boiler.rotation.z=Math.PI/2;boiler.position.x=-.5;box(2.6,3.5,3.4,M.red,3,2.15,0,g);box(2.1,.35,3.7,M.roof,3,4,0,g);cyl(.42,.62,2.2,10,M.dark,-2.35,4,0,g);const lamp=cyl(.18,.18,.35,8,mat(0xd6c27b,.55),-3.55,2.25,0,g);lamp.rotation.z=Math.PI/2;for(const x of [-2.8,-.7,1.5,3.1]){wheel(x,-1.65,.72,g);wheel(x,1.65,.72,g)}return g}
...
train.add(makeLocomotive());train.add(makeFreightCar(-10.8));train.add(makeFreightCar(-21.6));train.add(makeFreightCar(-32.4));
const trainParkX=stationX(-43);
train.position.set(trainParkX,0,-23);scene.add(train);
let trainState='parked',trainTimer=0;

function makeTruck(){const g=new THREE.Group();box(8,.5,3.5,M.dark,0,.72,0,g);box(3,2.5,3.2,M.olive,2.25,2,0,g);box(2.4,1.05,3.1,M.olive,4.5,1.45,0,g);box(4.3,.24,3.25,M.wood,-1.55,1.45,0,g);for(const z of [-1.55,1.55])for(const x of [-2.5,2.8])wheelTruck(x,z,g);box(1.5,.55,.12,mat(0x9ba39e,.35,.12),2.45,2.35,-1.63,g);box(1.5,.55,.12,mat(0x9ba39e,.35,.12),2.45,2.35,1.63,g);return g}
function wheelTruck(x,z,parent){const w=cyl(.72,.72,.42,12,M.dark,x,.72,z,parent);w.rotation.x=Math.PI/2}
const truck={group:makeTruck(),state:'station',cargo:[],departureAt:0,returnAt:0,speed:11};
truck.group.position.set(stationTruckX,0,8);truck.group.rotation.y=Math.PI;scene.add(truck.group);
...
const trainCargoSlots=[[-58,-22],[-54.8,-22],[-51.6,-22],[-69,-22],[-65.8,-22],[-62.6,-22],[-79.8,-22],[-76.6,-22],[-73.4,-22]].map(([x,z])=>[stationX(x),z]);
...
const storageSlots=[];for(let z=6.3;z<=13.7;z+=2.5)for(let x=51.5;x<=64.5;x+=2.8)storageSlots.push([depotX(x),z]);
let storageCursor=0;
...
const stationTruckZone={x0:stationX(-53),x1:stationX(-42),z0:3,z1:13};
const depotStorageZone={x0:depotX(48),x1:depotX(68),z0:3,z1:17};
...
navNext.addEventListener('click',()=>go('station'));navPrev.addEventListener('click',()=>go('depot'));
updateHUD();
spawnTrainCargo();
...
if(truck.state==='toDepot'){truck.group.rotation.y=Math.PI;truck.group.position.x=Math.max(depotTruckX,truck.group.position.x-truck.speed*dt);if(truck.group.position.x<=depotTruckX){truck.state='depot';truck.cargo.forEach(c=>c.state='truckDepot');notify('トラックが補給所へ到着');updateHUD()}}
if(truck.state==='depot'&&truck.cargo.length===0&&truck.returnAt&&now>=truck.returnAt){truck.state='returning';truck.returnAt=0;truck.group.rotation.y=0;notify('空トラックが貨物駅へ帰投');updateHUD()}
if(truck.state==='returning'){truck.group.position.x=Math.min(stationTruckX,truck.group.position.x+truck.speed*dt);if(truck.group.position.x>=stationTruckX){truck.state='station';truck.group.rotation.y=Math.PI;notify('トラックが貨物駅へ戻った');updateHUD()}}}
...
if(trainState==='arriving'){train.position.x-=18*dt;if(train.position.x<=trainParkX){train.position.x=trainParkX;trainState='parked';trainTimer=0;spawnTrainCargo()}}}
