(() => {
  ...
  function isPresent(point) {
    ...
    const v = point.display_value;
    if (v === null || v === undefined) return false;
    const s = String(v).trim();
    return s.length > 0 && !NON_VALUES.has(s) && !!point.source_url;
  }

  async function fetchVerifiedProfile(lookup) {
    if (!lookup) return null;
    const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_public_company_profile`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) throw new Error(`Verified company data: ${res.status}`);
  ...
  function group(profile) {
    ...
    for (const p of profile?.data_points || []) {
      if (!isPresent(p)) continue;
      const meta = FIELD_META[p.field_name];
      if (!meta) continue;
      buckets[meta[0]].push({ ...p, label: meta[1] });
    }
  ...
  function point(profile, fieldName) {
    return (profile?.data_points || []).find(p => p.field_name === fieldName && isPresent(p)) || null;
  }

  function uniqueSources(profile) {
    const seen = new Set();
    return (profile?.data_points || []).filter(isPresent).filter(p => {
  ...
  function quality(profile) {
    ...
    const sourceCoverageRate = verifiedFields ? Math.round(points.filter(p => p.source_url).length / verifiedFields * 100) : 0;
    const highConfidenceRate = verifiedFields ? Math.round(points.filter(p => p.confidence === "high").length / verifiedFields * 100) : 0;
    const dates = points.map(p => p.verified_at).filter(Boolean).sort();
    return {
      verifiedFields,
      sourceCoverageRate,
      highConfidenceRate,
